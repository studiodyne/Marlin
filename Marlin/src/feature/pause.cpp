...
static bool ensure_safe_temperature(const bool wait=true, const PauseMode mode=PAUSE_MODE_SAME) {
  ...
  while (marlin.wait_for_user && marlin.is_heating() && ABS(thermalManager.wholeDegHotend(active_extruder) - thermalManager.degTargetHotend(active_extruder)) > (TEMP_WINDOW))
    marlin.idle();
  marlin.heatup_done();
  marlin.wait_for_user = true;
