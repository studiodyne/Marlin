...
void GcodeSuite::M106() {
  ...
  if (FAN_IS_REDUNDANT(pfan)) return;
    ...
    /*SERIAL_ECHOPGM_P(
      PSTR("Parser.value: B="), LINEAR_UNIT(parser.intval('B') ),
        PSTR(" : C="), LINEAR_UNIT(parser.intval('C') ),
        PSTR(" : D="), LINEAR_UNIT(parser.intval('D') )
        );
    SERIAL_EOL();
    SERIAL_ECHOPGM_P(
      PSTR("Fan_speed_offset: B="), LINEAR_UNIT(thermalManager.fan_speed_offset_thr_min[pfan]),
        PSTR(" : C="), LINEAR_UNIT(thermalManager.fan_speed_offset[pfan]),
        PSTR(" : D="), LINEAR_UNIT(thermalManager.fan_speed_offset_thr_max[pfan])
        );
   SERIAL_EOL();*/
