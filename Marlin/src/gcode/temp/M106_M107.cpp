...
void GcodeSuite::M106() {
  ...
  if (FAN_IS_REDUNDANT(pfan)) return;
    ...
    thermalManager.fan_speed_modifier[pfan] = parser.intval('C');
    thermalManager.fan_speed_modifier_thr_min[pfan] = parser.intval('B');
    thermalManager.fan_speed_modifier_thr_max[pfan] = parser.intval('D');
  #endif
  ...
  #if ENABLED(FAN_SPEED_MODIFIER)
    if ( (speed > thermalManager.fan_speed_modifier_thr_min[pfan])  && (speed < thermalManager.fan_speed_modifier_thr_max[pfan])) speed += thermalManager. fan_speed_modifier[pfan];
  #endif
